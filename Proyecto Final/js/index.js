...
document.addEventListener('DOMContentLoaded', () => {
	...
	// Robust toggle: ignore repeated events that happen within a short timeframe
	function performNavToggle(e) {
		if (e && typeof e.preventDefault === 'function') e.preventDefault();
		if (!nav || !toggle) return;
		const now = Date.now();
		if (window.__lastNavToggleAt && (now - window.__lastNavToggleAt) < 350) return; // debounce rapid triggers
		window.__lastNavToggleAt = now;
	...
	if (toggle) {
		// pointerdown covers mouse and touch in modern browsers; keep passive:false to allow preventDefault
		toggle.addEventListener('pointerdown', (e) => performNavToggle(e), { passive: false });
		// legacy fallback
		toggle.addEventListener('touchstart', (e) => performNavToggle(e), { passive: false });
		// click as accessibility fallback (keyboard or older browsers)
		toggle.addEventListener('click', (e) => performNavToggle(e));
		// keyboard support
		toggle.addEventListener('keydown', (e) => { if (e.key === 'Enter' || e.key === ' ') { performNavToggle(e); } });
	}
